          ...
          ret?.dataArray?.map(async (txObj) => {
            if (txObj.method.includes("router::swapExact")) {
              ...
              const tokenInInfo = commonFiTokenList.find(
                (item) => item.tokenAddress === txObj.tokenPath[0]
              );
              console.log("tokenInInfo", tokenInInfo);
              const tokenOutInfo = commonFiTokenList.find(
                (item) => item.tokenAddress === txObj.tokenPath[1]
              );
              ...
              console.log("txObj.method.", txObj.method);
              console.log("txObj?.blockNumber.", txObj?.blockNumber);
            ...
            //       tokenContract: txObj?.data?.tokenContract,
            //       tokenSymbol,
            //       amount: formatTokenAmount(txObj?.data?.amount, +decimal),
            //       blockNumber: timeEvent,
            //       time: txObj?.createdTime,
            //       fromAddress: txObj?.fromAddress,
            //       toAddress: txObj?.toAddress,
