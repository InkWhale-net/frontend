        ...
        <IWCardOneColumn
          ...
          data={[
            ...
            },
            {
              ...
              content: `${formatNumDynDecimal(tokenBalance) || 0
                } ${tokenSymbol}`,
            },
          ]}
        />
...
const MyStakeRewardInfoToken = ({
  ...
  const validateStakeTokenLP = async () => {
    ...
    if (+LPtokenBalance < +LPTokenAmount) {
      toast.error("There is not enough balance!");
      return;
    }
    if (!(+LPTokenAmount <= availableStakeAmount)) {
  ...
  const stakeLPMutation = useMutation(async () => {
    try {
      return await stakeTokenLPHandler();
    } catch (error) {
  ...
  const unStakeLPMutation = useMutation(async () => {
    try {
      return await unstakeTokenLPHandler();
    } catch (error) {
      console.error("Error in stakeLPMutation:", error);
      throw error;
    }
  });
  async function unstakeTokenLPHandler() {
    if (!currentAccount) {
      toast.error(toastMessages.NO_WALLET);
      return;
    }
        ...
        >
          ...
          <IWCard mt="24px" w="full" variant="solid">
            ...
            >
              ...
              >
                <ConfirmModal
                  isLoading={stakeLPMutation.isLoading}
                  disableBtn={
                    !(availableStakeAmount > 0) ||
                    isPoolEnded(startTime, duration) ||
                    isPoolNotStart(startTime, duration) ||
                    !(LPTokenAmount?.length > 0) ||
                    stakeLPMutation.isLoading
                  }
                  action="stake"
                  buttonVariant="primary"
                  buttonLabel="Stake"
                  ...
                  onClick={() => stakeLPMutation.mutate()}
                ...
                <ConfirmModal
                  ...
                  disableBtn={
                    ...
                    !(LPTokenAmount > 0) ||
                    unStakeLPMutation.isLoading
                  }
                  action="unstake"
                  buttonVariant="primary"
                  buttonLabel="Unstake"
                  onValidate={onValidateUnstake}
                  onClick={() => unStakeLPMutation.mutate()}
                  message={formatMessageStakingPool(
                    "unstake",
                    LPTokenAmount,
                    lptokenSymbol,
